...
var randomUsernames = []string{
	"alice", "bob", "charlie", "david", "eve", "fred", "grace", "heidi", "ivan", "john", "karl", "laura", "mallory",
	"nina", "oscar", "peggy", "quinn", "rachel", "steve", "trent", "ursula", "victor", "wendy", "xander", "yvonne",
	"zacharias", "amber", "brian", "carol", "doug", "eric", "fiona", "george", "hanna", "ian", "jess", "kevin", "lisa",
	"mike", "nate", "oliver", "peter", "queen", "ron", "susan", "tim", "uma", "vicky", "walt", "xenia", "yasmin", "zoe",
}
...
func generateUsers(num int) []*store.User {
	...
	for i := 0; i < num; i++ {
		users[i] = &store.User{
			Username:  randomUsernames[i%len(randomUsernames)] + fmt.Sprintf("%d", i),
			Email:     randomUsernames[i%len(randomUsernames)] + fmt.Sprintf("%d@example.com", i),
			FirstName: randomUsernames[i%len(randomUsernames)],
			LastName:  "",
			Password:  "123123",
		}
	}
...
func generatePosts(num int, users []*store.User) []*store.Post {
	...
	for i := 0; i < num; i++ {
		...
		posts[i] = &store.Post{
			...
			Tags: []string{
				randomTags[rand.Intn(len(randomTags))],
				randomTags[rand.Intn(len(randomTags))],
			},
		}
	}
